...
class evol_grad():
    ...
    def save_molecule_to_file(self, positions: np.ndarray, atom_types: np.ndarray, 
        ...
        Args:
            ...
            atom_types: 原子类型索引 (n_atoms,) - 注意：这里是编码索引，需要通过atom_decoder解码
            output_path: 输出目录路径
            molecule_idx: 分子索引
            property_value: 属性值
            property_name: 属性名称
    ...
    def sample(self, fitness_score, sample_size):
        ...
        从种群中根据适应度分数采样
        Args:
            fitness_score: 种群中每个个体的适应度分数列表
            sample_size: 要采样的个体数量
        Returns:
            采样得到的个体索引列表
        """
        # 归一化适应度分数
    ...
    def run_evolution(self):
        ...
        self.means = means
        self.mads = mads
        target_tensor = [mean.detach() for mean in means]  # 默认使用数据集均值

        # 1. 初始化population - 从qm9_40000.npz中选择分子
        print('-'*30)
...
if __name__ == "__main__":
    ...
    print(f"参数 - t_add: {args.t_add}, 种群大小: {args.population_size}, 进化代数: {args.n_generations}")
    '''
   , ['gap','mu'], ['alpha', 'mu'],
     ['homo','lumo'],['lumo','gap'],
    ...
    for property_name in [ ['homo','gap'] ]:
        ...
        if isinstance(args.property, list):
            property_str = '_'.join(args.property)  # 将 ['alpha', 'mu'] 转换为 'alpha_mu'
        else:
            property_str = str(args.property)
        print(f"目标属性: {args.property}")
        # 更新实验名称
